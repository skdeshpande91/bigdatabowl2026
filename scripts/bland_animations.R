...
post <- get_posterior_params(paths = paths, 
                             rec_target = c(73.5, 47),
                             sigma = 1/36, tau = 1)
...
for(ix in 1:(n-1)){
  ...
  png(paste0("bland_animations/paths_", write_ix, ".png"), 
  ...
  for(i in 1:n_draws){
    ...
    lines(
      ...
      col = adjustcolor(oi_colors[5], alpha.f = 0.1), lwd = 0.1)
  ...
  points(27-paths$rec_y[1:ix], paths$rec_x[1:ix]-los, pch = 16, col = oi_colors[5], cex = 0.8)
  points(27-paths$def_y[1:ix], paths$def_x[1:ix]-los, pch = 16, col = oi_colors[6], cex = 0.8)
  
  points(27-paths$rec_y[ix], paths$rec_x[ix]-los, pch = 16, col = oi_colors[5], cex = 1)
  ...
  legend("bottomleft", bty = "n", cex = 1,
         legend = c("D. Bland", "J. Dotson", "S. Howell", "Ball"),
         pch = c(16, 16, 16, 15),
         col = oi_colors[c(6,5,1,8)])
  dev.off()
}
